...
-- Both functions are recreated whole rather than patched, because migrations
-- are append-only and the deployed body is whatever the last `create or
-- replace` said. The only change in each is the `private.enqueue_notification`
-- call and the locals feeding it; everything else is carried over verbatim
-- from `20260818040000_membership_proof_guards.sql` and
-- `20260806000000_admin_role_and_deletion_policy_updates.sql`.
