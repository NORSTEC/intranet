...
-- Merging folded the duplicate's name into the survivor one column at a time.
-- `full_name`, `first_name` and `last_name` are not three facts, they are one
-- fact written three ways, and a Google sign-in only ever fills the first of
-- them while an access request fills the other two. So merging a profile that
-- had been through an access request into a profile created by signing in
-- produced a person whose full name was one human and whose first name was
-- another — and the dashboard greets people by first name, so the very first
-- thing that profile's owner saw was somebody else's name.
--
-- The same mistake is in the avatar: `avatar_alt` describes `avatar_path`, and
-- coalescing them separately put a description of one person's face onto
-- another person's photograph. Both move as a set from here, and only a
-- profile that has none of a thing takes the duplicate's.
--
-- The other fields keep filling blanks, which is what a duplicate repair is
-- for: a study field or a phone number missing on one side and present on the
-- other is the same person's, described once. Names and faces are the two that
...
-- The profiles already broken by this, repaired rather than left for somebody
-- to notice. Deliberately narrow on both axes: only profiles a merge has
-- actually landed on, and only where a name part contradicts the whole rather
-- than merely being absent from it. A name nobody can edit from the interface
-- is not one to guess at, so the parts go and the full name — the one a Google
-- sign-in always sets — remains.
update public.people as person
set first_name = null,
    last_name = null
where person.full_name is not null
  and (
    (
      person.first_name is not null
      and position(lower(person.first_name) in lower(person.full_name)) = 0
    )
    or (
      person.last_name is not null
      and position(lower(person.last_name) in lower(person.full_name)) = 0
    )
  )
  and exists (
    select 1
    from public.audit_events as event
    where event.action = 'person.merged'
      and event.target_person_id = person.id
  );
