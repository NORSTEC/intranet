...
-- Merging quietly moved the surviving person's contact address. With no
-- explicit primary, `merge_people` cleared both people's primary flag and then
-- promoted whichever address had the oldest `created_at` — across both sets. A
-- duplicate imported before the person ever signed in therefore won, and the
-- address the portal shows other members, and the address a decline notice is
-- sent to, silently became one nobody had chosen. Portal management never
-- passes the parameter, so this was the only path a merge ever took.
--
-- The surviving person now keeps the address they already answered on. The
-- oldest-address fallback survives for the single case it was ever right for:
-- a profile that had no address of its own.
--
-- Along with it, the two structural facts a merge must not be able to break,
-- asserted where they can still be rolled back. They are cheap, they run once
-- per merge, and both of them have been one refactor away from silently
-- failing: the primary flag is a partial unique index that permits *no*
-- primary just as happily as one, and the two-account limit lives in three
-- separate RPCs and in none of the constraints.
