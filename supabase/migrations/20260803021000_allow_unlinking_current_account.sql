...
-- Linking an alternative Google account leaves the browser signed in as that
-- new account, so refusing to unlink the current session's account stranded
-- people on the one account they most likely wanted to undo. Unlinking your
-- own session is safe: the primary account is still protected, and at least
-- one other account always remains to sign back in with. The caller signs
-- the session out afterwards.
